...
function createComposerRuntime(deps = {}) {
	...
	function insertComposerText(value) {
		...
		if (!input || !addition || state.composerComposing || input.contentEditable === "false" || input.getAttribute("aria-disabled") === "true") return null;
		...
		const placeholder = container.nodeType === 1 && container.childNodes.length === 1 && container.firstChild.nodeName === "BR" ? container.firstChild : null;
  ...
  `).join("");
	...
	async function sendMessage(event) {
		...
		const clientSubmissionId = /^[A-Za-z0-9._:-]{1,128}$/.test(requestedSubmissionId) ? requestedSubmissionId : createSubmissionId();
		...
		try {
			...
			if (requestedSubmissionId.startsWith("voxspark-") && err && err.code === "voxspark_submission_outcome_unknown") throw err;
		} finally {
			finishSendProgressWatchdog();
			state.composerBusy = false;
			updateComposerControls();
...
function voiceStatusView(voice, sessionId, connected = true) {
	const reasons = {
		bridge_disconnected: "语音服务重连中",
		state_unavailable: "等待语音状态",
		no_active_composer: "请选中要接收语音的 Session",
		target_syncing: "正在同步语音目标",
		composer_unavailable: "当前输入框暂不可接收语音",
		box_disconnected: "BOX 未连接",
		ready: "BOX 已就绪"
	};
	...
	const capture = captures.find((item) => item && item.state === "recording") || own.find(active) || captures.find(active) || ownCapture;
	const targetId = capture ? capture.session_id : target && target.session_id;
	const targetTitle = text(capture && capture.session_title || (target && target.session_id === targetId ? target.title : "")).slice(0, 80);
	const labels = {
		recording: "BOX 录音中",
		transcribing: "识别中",
		polishing: "整理中",
		waitingComposer: "等待写入输入框",
		ready: "已写入输入框",
		failed: "识别失败，请在 BOX 重试或丢弃"
	};
	const stage = capture && labels[capture.state] ? capture.state : "";
	const milliseconds = capture && Number.isSafeInteger(capture.duration_ms) ? Math.min(54e4, Math.max(0, capture.duration_ms)) : 0;
	const seconds = Math.floor(milliseconds / 1e3);
	const details = [];
	if (ownCapture && ownCapture !== capture && labels[ownCapture.state]) details.push(`当前 Session：${labels[ownCapture.state]}`);
	if (stage && reason !== "ready") details.push(reasons[reason] || reasons.state_unavailable);
	return {
		reason,
		target: targetTitle ? `语音发送到：${targetTitle}` : capture ? `语音发送到：${targetId === sessionId ? "当前 Session" : "另一个 Session"}` : "语音输入",
...
function createVoxSparkSurfaceHostRuntime(deps = {}) {
	...
	const captureRows = /* @__PURE__ */ new Map();
	const captureRequestIds = /* @__PURE__ */ new Map();
	...
	let queueNotice = "";
	const queueRows = /* @__PURE__ */ new Map();
	...
	function renderVoiceStatus() {
		...
		const fresh = relayConnected && now() - voiceReceivedAt < 3e3;
		const view = voiceStatusView(voice, currentComposerThreadId(), fresh);
		const values = {
			voxsparkVoiceTarget: view.target,
	...
	function recoveryAvailable() {
		return !stopped && relayConnected && now() - voiceReceivedAt < 3e3 && surfaceCanArm() && currentContext?.sessionId === currentComposerThreadId() && voice?.snapshot?.target?.session_id === currentComposerThreadId() && ![
			"bridge_disconnected",
			"target_syncing",
			"state_unavailable",
			"no_active_composer"
		].includes(voice?.reason);
	}
	function renderCaptureRecovery() {
		...
		for (const [id, row] of captureRows) if (!ids.has(id)) {
			row.element.remove();
			captureRows.delete(id);
		}
		for (const item of items) {
			let row = captureRows.get(item.capture_id);
			if (!row) {
				const element = document.createElement("li");
				element.className = "voxspark-recovery-item";
				const label = document.createElement("span");
				const actions = document.createElement("div");
				actions.className = "voxspark-queue-actions";
				const buttons = {};
				for (const [operation, title] of Object.entries({
					retry: "重试识别",
					discard: "丢弃录音"
				})) {
					...
					button.dataset.captureAction = operation;
					button.dataset.captureId = item.capture_id;
				...
				captureRows.set(item.capture_id, row);
			}
			const recovery = item.recovery;
			const remaining = recovery?.retryable ? Math.max(0, (recovery.expires_at || 0) - now()) : 0;
			const duration = Math.ceil(item.duration_ms / 1e3);
			row.label.textContent = `${duration} 秒录音 · ` + (!recovery ? "当前 Bridge 不支持网页恢复" : remaining > 0 ? `识别失败，可重试（剩余 ${Math.ceil(remaining / 6e4)} 分钟）` : "录音已无法恢复，请丢弃后重新录音");
			for (const [operation, button] of Object.entries(row.buttons)) button.disabled = !captureRequest || !recoveryAvailable() || !recovery || Boolean(captureOperation) || operation === "retry" && remaining <= 0;
		}
	...
	async function manageCapture(operation, captureId) {
		...
		const key = [
			snapshot.bridge_epoch,
			captureId,
			capture.recovery.revision,
			operation
		].join(":");
		...
		const request = {
			sessionId,
			epoch: relayServiceEpoch
		};
		captureOperation = request;
		captureNotice = null;
		renderCaptureRecovery();
		try {
			const result = await captureRequest(operation, {
				client_id: clientId,
				service_epoch: relayServiceEpoch,
				request_id: captureRequestIds.get(key),
				bridge_epoch: snapshot.bridge_epoch,
				session_id: sessionId,
				capture_id: captureId,
				recovery_revision: capture.recovery.revision
			});
			...
			captureNotice = {
				sessionId,
				text: result?.ok ? operation === "retry" ? "已开始重试，识别完成后写入此 Session 的草稿。" : "已丢弃录音。" : result?.code === "recovery_expired" ? "录音已过期，请丢弃后重新录音。" : "操作未确认，请查看更新后的录音状态；再次点击会核对同一请求。"
			};
			return result?.ok === true;
		} catch (_) {
			if (!stopped && currentComposerThreadId() === sessionId && relayServiceEpoch === request.epoch) captureNotice = {
				sessionId,
				text: "操作未确认，请查看更新后的录音状态；再次点击会核对同一请求。"
			};
			return false;
		} finally {
			if (captureOperation === request) {
	...
	function normalizeQueueSnapshot(value) {
		...
		return value.slice(0, 8).map((item) => ({
			...
			status: [
				...
				"failed",
				"unknown"
			].includes(item && item.status) ? item.status : "",
	...
	function renderQueue() {
		...
		const available = now() - queueReceivedAt < 3e3;
		$("voxsparkQueueNotice").textContent = !available && items.length ? "暂时无法连接队列，恢复连接后可操作。" : queueNotice || "仅当前 Session；空闲且输入框为空时依次提交。已提交不代表回复完成。";
		...
		for (const item of items.sort((a, b) => a.createdAt - b.createdAt)) {
			...
			if (!row) {
				const element = document.createElement("li");
				element.className = "voxspark-queue-item";
				const title = document.createElement("span");
				const actions = document.createElement("div");
				actions.className = "voxspark-queue-actions";
				const preview = document.createElement("p");
				preview.className = "voxspark-queue-preview";
				const buttons = {};
				for (const [operation, label] of Object.entries({
					...
					button.dataset.queueAction = operation;
				...
				queueRows.set(item.queueId, row);
			}
			...
			const allowed = {
				steer: item.status === "queued",
				cancel: ["queued", "failed"].includes(item.status),
			...
			for (const [operation, button] of Object.entries(row.buttons)) {
				button.hidden = !allowed[operation];
				button.disabled = !available || Boolean(queueOperation) || queueFlushing || operation === "steer" && (!text(composerTargetActiveTurnId()) || currentContext?.approvalPending);
			}
	...
	async function manageQueue(operation, queueId) {
		if (stopped || queueOperation || queueFlushing || !queueRequest || !currentContext || now() - queueReceivedAt >= 3e3) return false;
		const activeTurnId = text(composerTargetActiveTurnId());
		if (operation === "steer" && (!activeTurnId || currentContext.approvalPending || !queuedDrafts.some((item) => item.queueId === queueId && item.status === "queued"))) return false;
		const request = {
			sessionId: currentContext.sessionId,
			revision: ++queueRevision,
			activeTurnId,
			thread: { ...composerTargetThread() || {} },
			serviceEpoch: relayServiceEpoch
		};
		...
		try {
			...
			const result = await queueRequest(operation, {
				client_id: clientId,
				service_epoch: relayServiceEpoch,
			...
			if (operation === "reconcile" || operation === "retry") queueNotice = result.queue?.status === "unknown" ? "仍无法确认提交结果；请查看本 Session 的消息，系统不会自动重发。" : result.queue?.status === "submitted" ? "已核实提交成功，可在本 Session 查看回复。" : "已重新排队。";
			return true;
		} catch (_) {
			if (!stopped && queueOperation === request && currentComposerThreadId() === request.sessionId) queueNotice = "操作未确认，请按队列当前状态处理。";
			return false;
		} finally {
			if (queueOperation === request) {
				queueRevision += 1;
				queueOperation = null;
				renderQueue();
				syncContext();
	...
	async function steerQueuedDraft(request, queueId) {
		const payload = {
			client_id: clientId,
			service_epoch: request.serviceEpoch,
			session_id: request.sessionId,
			queue_id: queueId
		};
		...
		try {
			if (stopped || queueOperation !== request || currentComposerThreadId() !== request.sessionId || text(composerTargetActiveTurnId()) !== request.activeTurnId || currentContext?.approvalPending) throw Error("queue_target_changed");
			sendStarted = true;
			await sendDraft({
			...
			outcome = error?.code === "voxspark_submission_outcome_unknown" || sendStarted && !Number(error?.status || error?.statusCode) ? "unknown" : "failed";
		}
		const completed = await queueRequest("complete", {
			...payload,
			lease_token: claimed.lease_token,
			outcome,
			error_code: outcome === "unknown" ? "action_outcome_unknown" : outcome === "failed" ? "queue_steer_failed" : ""
		});
		if (!stopped && queueOperation === request && currentComposerThreadId() === request.sessionId) {
			mergeQueueEntry(completed?.queue);
			if (!completed?.ok || outcome === "unknown") queueNotice = "引导结果尚未确认，系统不会自动重发；请核对提交结果。";
			else if (outcome === "succeeded") {
				queueTurnGate = "awaiting_idle";
				queueNotice = "已转为引导，发送给当前正在执行的任务。";
			} else queueNotice = "引导未成功，消息已保留；可重试排队后再转为引导。";
		}
	...
	function resetQueueView() {
		...
		queueOperation = null;
		queuedDrafts = [];
		queueReceivedAt = Number.NEGATIVE_INFINITY;
		queueNotice = "";
		const panel = $("voxsparkQueue");
	...
	async function relayContext(options = {}) {
		...
		if (activeRelay) {
			...
			diagnostic("context_relay_superseded", {
				syncId: activeRelay.syncId,
				requestStartedAt: activeRelay.startedAt,
				clientAt: now(),
		...
		try {
			...
			const relayDetail = {
				...
				clientAt: responseAt,
				relayMs: Math.max(0, responseAt - request.startedAt),
				surfaceRevision: request.revision
			};
			if (request.observe || relayDetail.relayMs >= 1e3 || result && Array.isArray(result.commands) && result.commands.length || request.acknowledgedSequences.length) diagnostic("context_relay_completed", {
				...relayDetail,
			...
			if (voice?.snapshot) {
				if (recoveryLastSnapshot && recoveryLastSnapshot.bridge_epoch !== voice.snapshot.bridge_epoch && recoveryLastSnapshot.captures.some((item) => item.state === "failed" && item.session_id === currentComposerThreadId())) captureNotice = {
					sessionId: currentComposerThreadId(),
					text: "Bridge 已重启，之前失败的录音已无法恢复，请重新录音。"
				};
			...
			for (const command of commands) {
				...
				diagnostic("command_received", {
					...relayDetail,
					clientAt: now(),
					...traceDetails(command.message, command.sequence)
				});
				if (await handleMessage(command.message, {
			...
			diagnostic("bridge_relay_error", {
				...
				errorKind: request.responseReceived ? "processing_failed" : String(error && error.message || "").startsWith("Request timed out:") ? "timeout" : request.controller.signal.aborted ? "cancelled" : "request_failed",
				acknowledgedCount: request.acknowledgedSequences.length
			});
			return false;
		} finally {
	...
	function syncContext(options = {}) {
		...
		const supersede = Boolean(activeRelay && (activeRelay.sessionId !== currentContext.sessionId || activeRelay.title !== currentContext.title));
	...
	function replaceComposer(message, metadata = {}) {
		const detail = {
			...
			syncId: traceId(metadata.syncId),
			clientAt: now()
		};
		...
		if (!inserted) {
			...
			diagnostic("replace_insert_failed", {
				...detail,
				clientAt: now()
			});
			return COMMAND_RETRY;
	...
	async function flushQueuedDrafts() {
		...
		const queued = queuedDrafts.find((item) => item.sessionId === currentContext.sessionId && item.status === "queued");
		...
		try {
			...
			if (!claimed || !claimed.ok || !text(claimed.text) || !traceId(claimed.lease_token)) return false;
			mergeQueueEntry(claimed.queue);
			let outcome = "succeeded";
			let errorCode = "";
			try {
	...
	async function handleAction(message) {
		...
		if (action === "submit" || action === "steer") return submitDraft(draft, action, { actionId: traceId(message.action_id) });
		return COMMAND_DISCARD;
	}
...
//#endregion
//#region frontend/native/voxspark-lexicon-runtime.mjs
var LEXICON_ERRORS = Object.freeze({
...
function createVoxSparkLexiconRuntime(deps = {}) {
	...
	function notice(text, error = false) {
		const status = $("voxsparkLexiconStatus");
		status.textContent = text;
	...
	function errorText(code) {
		return LEXICON_ERRORS[code] || "操作未完成，请刷新词库后重试。";
	}
	...
	function render() {
		const list = $("voxsparkLexiconItems");
		...
		$("voxsparkLexiconCount").textContent = snapshot ? `${entries.length} 个词条 · ${entries.filter((item) => item.enabled).length} 个启用${query ? ` · 找到 ${filtered.length} 个` : ""}` : "";
		...
		for (const item of filtered.sort((a, b) => b.updatedAt - a.updatedAt || a.text.localeCompare(b.text))) {
			const row = node("li", null, "voxspark-lexicon-item");
			row.dataset.term = item.text;
			...
			if (item.aliases.length) {
				...
				for (const alias of item.aliases) {
					const chip = node("li");
					chip.appendChild(node("span", `${alias} → ${item.text}`));
					const remove = button("移除", () => mutate("unalias", {
						text: item.text,
						alias
					}));
					remove.setAttribute("aria-label", `移除 ${item.text} 的错写 ${alias}`);
			...
			actions.appendChild(button("补充错写", () => {
				$("voxsparkLexiconTerm").value = item.text;
				$("voxsparkLexiconAlias").value = "";
			...
			if (removeTarget === item.text) {
				...
				actions.appendChild(button("取消", () => {
					removeTarget = "";
					render();
				}));
			} else actions.appendChild(button("删除", () => {
				removeTarget = item.text;
				render();
			}));
	...
	async function load(preserveNotice = "") {
		...
		setBusy(true);
		if (!preserveNotice) notice("正在读取词库…");
		try {
			...
			bridgeEpoch = result.bridge_epoch;
			removeTarget = "";
			...
			if (dialog.open && currentGeneration === generation && sequence === loadSequence) {
				snapshot = null;
				bridgeEpoch = "";
				notice(errorText(error?.code), true);
			}
			return false;
		} finally {
			if (dialog.open && currentGeneration === generation && sequence === loadSequence) {
	...
	async function mutate(action, fields) {
		...
		const payload = {
			...fields,
			request_id: `lexicon:${requestPrefix}:${++requestSequence}`,
			bridge_epoch: bridgeEpoch,
		...
		try {
			...
			if (!result?.ok) {
				...
				if ([
					"lexicon_revision_conflict",
					"lexicon_stale_epoch",
					"lexicon_outcome_unknown",
					"lexicon_term_not_found"
				].includes(result?.code)) await load(message);
			...
			bridgeEpoch = result.bridge_epoch;
			removeTarget = "";
			if (action === "add" && $("voxsparkLexiconTerm").value.trim() === fields.text && $("voxsparkLexiconAlias").value.trim() === (fields.aliases?.[0] || "")) {
				$("voxsparkLexiconTerm").value = "";
				$("voxsparkLexiconAlias").value = "";
			}
			notice("已保存，下次录音生效。");
			return true;
			...
			if (dialog.open && currentGeneration === generation) {
				setBusy(false);
				render();
			}
		}
	...
	function initialize() {
		...
		$("voxsparkLexiconRefresh").addEventListener("click", () => {
			if (!busy) load();
		});
		...
		$("voxsparkLexiconForm").addEventListener("submit", (event) => {
			...
			if ([text, ...alias ? [alias] : []].some((value) => [...value].length < 2 || [...value].length > 64)) {
				notice(errorText("lexicon_invalid_term"), true);
				return;
			}
			mutate("add", {
				text,
...
var lexiconApi = { createVoxSparkLexiconRuntime };
var lexiconRoot = typeof globalThis !== "undefined" ? globalThis : window;
lexiconRoot.CodexVoxSparkLexiconRuntime = lexiconApi;
//#endregion
...
var moduleDefinitions = [
	...
	},
	{
		...
		"source": "public/voxspark-lexicon-runtime.js",
		"nativeSource": "frontend/native/voxspark-lexicon-runtime.mjs",
		"globalName": "CodexVoxSparkLexiconRuntime",
		"expectedFunctions": ["createVoxSparkLexiconRuntime"],
		"assetPath": "/voxspark-lexicon-runtime.js",
...
var moduleApis = {
	...
	"voxspark-lexicon-runtime": lexiconApi
};
