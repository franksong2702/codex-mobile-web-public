...
test("receipt lookup uses the execution ledger, is Session scoped, and does not write", async (t) => {
  const { createService, storePath } = fixture(t);
  const service = createService();
  assert.deepEqual(service.readVoxSparkSubmissionReceipt("session-a", "voxspark-id.a:1"), { phase: "missing" });
  let finish;
  const execution = service.runMessageSubmissionOnce(["client:session-a:voxspark-id.a:1"], [], () => new Promise(resolve => { finish = resolve; }));
  await Promise.resolve();
  assert.deepEqual(service.readVoxSparkSubmissionReceipt("session-a", "voxspark-id.a:1"), { phase: "executing" });
  finish({ ok: true, turnId: "turn-a", body: "private payload" }); await execution;
  const restarted = createService();
  const before = fs.readFileSync(storePath, "utf8");
  assert.deepEqual(restarted.readVoxSparkSubmissionReceipt("session-a", "voxspark-id.a:1"), { phase: "succeeded" });
  assert.deepEqual(restarted.readVoxSparkSubmissionReceipt("session-b", "voxspark-id.a:1"), { phase: "missing" });
  assert.equal(fs.readFileSync(storePath, "utf8"), before);
  assert.doesNotMatch(before, /private payload/);
});

test("receipt lookup treats unreadable storage as unavailable, and a missing receipt never as success", (t) => {
  const { createService, storePath } = fixture(t);
  fs.writeFileSync(storePath, "broken-json");
  assert.deepEqual(createService().readVoxSparkSubmissionReceipt("session-a", "voxspark-action-a"), { phase: "unavailable" });
  assert.deepEqual(createMediaFileService().readVoxSparkSubmissionReceipt("session-a", "voxspark-action-a"), { phase: "unavailable" });
});
