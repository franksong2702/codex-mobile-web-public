...
var voxsparkSurfaceHostRuntime = null;
var voxsparkLexiconRuntime = null;
...
function initializeComposerRuntimeWiring() {
	...
	composerRuntime = composerRuntimeApi.createComposerRuntime({
		...
		normalizeOptionList,
		normalizeThreadGoal,
		onComposerSubmitted: (submission) => voxsparkSurfaceHostRuntime && voxsparkSurfaceHostRuntime.handleComposerSubmission(submission),
...
function initializeVoxSparkSurfaceHostRuntimeWiring() {
	...
	voxsparkSurfaceHostRuntime = surfaceHostApi.createVoxSparkSurfaceHostRuntime({
		...
		queueRequest: (operation, payload) => api(`/api/voxspark/surface/queue/${operation}`, {
			method: "POST",
			body: JSON.stringify(payload),
			timeoutMs: operation === "complete" ? 1e4 : 4e3
		}),
		$,
		currentComposerThreadId,
		composerTargetThread,
		...
		approvalPending: (threadId, turnId) => Boolean(threadId && turnId && approvalsForTurn(threadId, turnId).some((request) => isApprovalActive(request))),
		report: (code, detail) => postClientEvent("voxspark_surface_host", {
			...
			errorKind: [
				...
				"request_failed",
				"processing_failed"
			].includes(detail && detail.errorKind) ? detail.errorKind : "",
			...
			commandSequence: Number.isInteger(detail && detail.commandSequence) ? detail.commandSequence : 0
		})
	});
...
function initializeVoxSparkLexiconRuntimeWiring() {
	...
	voxsparkLexiconRuntime = factory({
		document,
		$,
...
function wireUi() {
	...
	document.addEventListener("focusout", () => {
		updateViewportVars();
		updateComposerHeightVar();
		scheduleVisualRecovery("focusout", 160, {
			render: false,
			heavy: false,
			delays: [160, 420]
		});
...
async function start() {
	...
	if (config.voxspark && config.voxspark.enabled && window.voxsparkSurfaceHostRuntime) {
		if (typeof window.voxsparkSurfaceHostRuntime.configurePolishContextConsent === "function") window.voxsparkSurfaceHostRuntime.configurePolishContextConsent(config.voxspark.polishContextConsent);
		if (!window.voxsparkSurfaceHostRuntime.readState().enabled) window.voxsparkSurfaceHostRuntime.configureBridgeUrl(config.voxspark.bridgeUrl);
	}
