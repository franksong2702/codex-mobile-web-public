"use strict";
const { normalizeLexiconDocument, normalizeLexiconRequest, token: lexiconToken } = require("./voxspark-lexicon-state");

const crypto = require("node:crypto");
const { normalizeVoiceState } = require("./voxspark-voice-state");
...
function browserQueueEntry(entry) {
  return entry ? { ...publicQueueEntry(entry), text: entry.text || "" } : null;
}
...
function createVoxSparkSurfaceHostService(options = {}) {
  ...
  const readSubmissionReceipt = typeof options.readSubmissionReceipt === "function"
    ? options.readSubmissionReceipt : () => ({ phase: "unavailable" });
  ...
  const clientSurfaceRevisions = new Map();
  ...
  function logCommand(event, details = {}) {
    ...
    logger.info(`[voxspark] surface command ${event}`, JSON.stringify({ ...details, serverAt: now() }));
  }
  ...
  function enqueueQueue(input = {}) {
    ...
    if (existing) {
      ...
      return matches
        ? { ok: true, queue: browserQueueEntry(existing), deduplicated: true }
    ...
    return { ok: true, queue: browserQueueEntry(entry), deduplicated: false };
  }
  ...
  function claimQueue(input = {}) {
    ...
    const clientId = traceId(input.client_id);
    const sessionId = boundedText(input.session_id, 128);
    const mode = input.mode == null ? "submit" : input.mode;
    if (!["submit", "steer"].includes(mode)) return { ok: false, code: "invalid_queue_mode" };
    if (!queueId || !queueSessionMatches(input) || !target || target.expiresAt <= now()
      ...
      || !target.context.composer.focused || target.context.turn.state !== (mode === "steer" ? "running" : "idle")
    ...
    if (entry.status !== "queued") {
      return { ok: false, code: `queue_${entry.status}`, queue: browserQueueEntry(entry) };
    }
    if (queueEntries.some((item) => item.sessionId === sessionId
      && item.queueId !== queueId && (mode === "steer" ? ["leased"] : ["leased", "submitted", "processing"]).includes(item.status))) {
  ...
  function completeQueue(input = {}) {
    ...
    return { ok: true, queue: browserQueueEntry(entry) };
  }

  function cancelQueue(input = {}) {
    trimQueue();
    const access = queueSnapshot(input);
    ...
    if (entry.status === "failed") {
      let receipt;
      try { receipt = readSubmissionReceipt(entry.sessionId, entry.clientSubmissionId); } catch (_) {}
      if (!receipt || !["missing", "succeeded", "executing", "unknown"].includes(receipt.phase)) {
        return { ok: false, code: "queue_receipt_unavailable" };
      }
      if (receipt.phase !== "missing") {
        const reconciled = recoverQueue(input, false);
        return reconciled.ok ? { ok: false, code: "queue_not_cancellable", queue: reconciled.queue } : reconciled;
      }
    }
    const previous = { ...entry };
  ...
  function queueSnapshot(input = {}) {
    ...
    return {
      ok: true,
      items: queueEntriesForSession(sessionId).map(browserQueueEntry),
    };
  }

  function inspectQueue(input = {}) {
    const access = queueSnapshot(input);
    if (!access.ok) return access;
    const entry = queueEntriesForSession(input.session_id).find((item) => item.queueId === traceId(input.queue_id));
    if (!entry) return { ok: false, code: "queue_not_found" };
    // Legacy explicit preview endpoint; Session-scoped polls also carry the full body.
    return { ok: true, queue: browserQueueEntry(entry), preview: entry.text.slice(0, 500), truncated: entry.text.length > 500 };
  }

  function recoverQueue(input, retry) {
    const access = queueSnapshot(input);
    if (!access.ok) return access;
    const entry = queueEntriesForSession(input.session_id).find((item) => item.queueId === traceId(input.queue_id));
    if (!entry) return { ok: false, code: "queue_not_found" };
    if (retry ? entry.status !== "failed" : !["failed", "unknown"].includes(entry.status)) {
      return { ok: false, code: "queue_not_recoverable" };
    }
    let receipt;
    try { receipt = readSubmissionReceipt(entry.sessionId, entry.clientSubmissionId); } catch (_) {}
    if (!receipt || !["succeeded", "executing", "unknown", "missing"].includes(receipt.phase)) {
      return { ok: false, code: "queue_receipt_unavailable" };
    }
    const previous = { ...entry };
    if (receipt.phase === "succeeded") {
      ...
      entry.text = "";
      entry.errorCode = "";
    } else if (retry && receipt.phase === "missing" && entry.text) {
      entry.status = "queued";
      entry.errorCode = "";
    } else {
      entry.status = "unknown";
      entry.errorCode = "action_outcome_unknown";
    }
    entry.updatedAt = now();
    if (!persistQueue()) {
      Object.assign(entry, previous);
    ...
    queueTargetChanged(entry.sessionId);
    return { ok: true, queue: browserQueueEntry(entry), outcome_confirmed: receipt.phase === "succeeded" };
  }

  function retryQueue(input = {}) { return recoverQueue(input, true); }
  function reconcileQueue(input = {}) { return recoverQueue(input, false); }
  ...
  function finishCaptureRequest(id, result) {
    const pending = captureRequests.get(id);
    if (!pending) return;
  ...
  function clearCaptureRequests() {
    for (const id of captureRequests.keys()) finishCaptureRequest(id, { ok: false, code: "capture_outcome_unknown" });
  }

  async function recoverCapture(input = {}) {
    const exactId = (value, max) => typeof value === "string" && value.length > 0 && value.length <= max
      && !/[\u0000-\u001f\u007f]/.test(value) ? value : "";
    const requestId = exactId(input.request_id, 128);
    const captureId = exactId(input.capture_id, 64);
    const sessionId = exactId(input.session_id, 128);
    if (!requestId || !captureId || !sessionId || !["retry", "discard"].includes(input.action)
      || !Number.isSafeInteger(input.recovery_revision) || input.recovery_revision < 1) return { ok: false, code: "invalid_capture_action" };
    if (input.service_epoch !== serviceEpoch || !target || target.expiresAt <= now()
      || target.clientId !== input.client_id || target.context.session.id !== sessionId
      || !target.context.composer.focused) return { ok: false, code: "capture_target_unavailable" };
    if (!socketIsOpen() || !voiceSnapshot || voiceSnapshot.bridge_epoch !== input.bridge_epoch
      || voiceSnapshot.target?.session_id !== sessionId
      || voiceSnapshot.target.context_revision !== target.contextRevision) return { ok: false, code: "stale_context" };
    const capture = voiceSnapshot.captures.find(item => item.capture_id === captureId && item.session_id === sessionId);
    if (!capture || capture.state !== "failed" || capture.recovery?.revision !== input.recovery_revision) return { ok: false, code: "capture_not_failed" };
    if (input.action === "retry" && (!capture.recovery.retryable || capture.recovery.expires_at <= now())) return { ok: false, code: "recovery_expired" };
    if (captureRequests.has(requestId) || captureRequests.size >= 8) return { ok: false, code: "capture_action_pending" };
    return new Promise(resolve => {
      const pending = { resolve, captureId, epoch: input.bridge_epoch,
        timer: setTimeoutFn(() => finishCaptureRequest(requestId, { ok: false, code: "capture_outcome_unknown" }), 3000) };
      captureRequests.set(requestId, pending);
      try {
        if (!send({ type: "host.capture.action", request_id: requestId, bridge_epoch: input.bridge_epoch,
          context_revision: target.contextRevision, session_id: sessionId, capture_id: captureId,
          recovery_revision: input.recovery_revision, action: input.action })) clearCaptureRequests();
      } catch (_) { finishCaptureRequest(requestId, { ok: false, code: "capture_outcome_unknown" }); }
    });
  }

  function finishLexiconRequest(id, result) {
    const pending = lexiconRequests.get(id);
    if (!pending) return;
    clearTimeoutFn(pending.timer);
    lexiconRequests.delete(id);
    pending.resolve(result);
  }

  function clearLexiconRequests() {
    for (const [id, pending] of lexiconRequests) finishLexiconRequest(id, { ok: false,
      code: pending.message.action === "list" ? "lexicon_disconnected" : "lexicon_outcome_unknown" });
  }

  function sendLexiconRequests() {
    if (!socketIsOpen()) return;
    for (const [id, pending] of lexiconRequests) {
      if (pending.sent) continue;
      pending.sent = true;
      pending.socket = socket;
      try { send(pending.message); }
      catch (_) { finishLexiconRequest(id, { ok: false, code: "lexicon_outcome_unknown" }); }
    }
  }

  async function manageLexicon(input = {}) {
    const request = normalizeLexiconRequest(input);
    if (!request) return { ok: false, code: "lexicon_invalid_request" };
    if (stopped || !(bridgeUrl || defaultBridgeUrl)) return { ok: false, code: "lexicon_unavailable" };
    const requestId = request.request_id || `lexicon:${crypto.randomUUID()}`;
    if (lexiconRequests.has(requestId) || lexiconRequests.size >= 8) return { ok: false, code: "lexicon_busy" };
    if (!configure(bridgeUrl || defaultBridgeUrl)) return { ok: false, code: "lexicon_disconnected" };
    return new Promise(resolve => {
      const pending = { resolve, sent: false, socket: null,
        message: { type: "host.lexicon.request", ...request, request_id: requestId },
        timer: setTimeoutFn(() => finishLexiconRequest(requestId, { ok: false,
          code: request.action === "list" ? "lexicon_disconnected" : "lexicon_outcome_unknown" }), 4000) };
      lexiconRequests.set(requestId, pending);
      sendLexiconRequests();
    });
  }
  ...
  function handleBridgeMessage(event) {
    ...
    if (message.type === "bridge.lexicon.result") {
      const pending = lexiconRequests.get(message.request_id);
      if (!pending || pending.socket !== socket || !lexiconToken(message.bridge_epoch, 64)
        || !["succeeded", "rejected", "unknown"].includes(message.outcome)) return;
      if (message.outcome === "succeeded") {
        const document = normalizeLexiconDocument(message.document);
        if (!document || (pending.message.action !== "list" && message.bridge_epoch !== pending.message.bridge_epoch)) return;
        finishLexiconRequest(message.request_id, { ok: true, bridge_epoch: message.bridge_epoch, document });
      } else {
        finishLexiconRequest(message.request_id, { ok: false, code: message.outcome === "unknown"
          ? "lexicon_outcome_unknown" : /^lexicon_[a-z_]{1,60}$/.test(message.error_code || "")
          ? message.error_code : "lexicon_rejected" });
      }
      return;
    }
    if (message.type === "bridge.capture.action.result") {
      const pending = captureRequests.get(message.request_id);
      if (!pending || pending.epoch !== message.bridge_epoch || pending.captureId !== message.capture_id
        || !["accepted", "rejected"].includes(message.outcome)) return;
      finishCaptureRequest(message.request_id, { ok: message.outcome === "accepted",
        code: message.outcome === "accepted" ? null : traceId(message.error_code) || "capture_action_rejected" });
      return;
    }
    if (message.type === "bridge.voice.state") {
      const next = normalizeVoiceState(message);
      if (next && (!voiceSnapshot || (next.bridge_epoch === voiceSnapshot.bridge_epoch
        && next.sequence > voiceSnapshot.sequence))) voiceSnapshot = next;
      return;
    }
  ...
  function connect() {
    ...
    nextSocket.addEventListener("message", (event) => {
      if (socket === nextSocket) handleBridgeMessage(event);
    });
  ...
  function publish(input = {}) {
    ...
    const previous = clientSurfaceRevisions.get(clientId)
      || (target && target.clientId === clientId ? {
        revision: target.surfaceRevision, sessionId: target.context.session.id,
      } : null);
    if (previous && (surfaceRevision < previous.revision
      || (surfaceRevision === previous.revision && context.session.id !== previous.sessionId))) {
      // Abort only stops waiting in the browser; an older POST may still arrive.
      // Reject it before changing ownership, URL, commands or draft receipts.
      return { ok: true, stale_context: true, connected: socketIsOpen(),
        service_epoch: serviceEpoch, context_revision: target?.contextRevision || 0,
        commands: [], accepted_command_sequences: [], accepted_result_ids: [] };
    }
    const urlAccepted = configure(input.bridge_url || defaultBridgeUrl);
    if (!urlAccepted) return { ok: false, code: "invalid_bridge_url" };
    clientSurfaceRevisions.delete(clientId);
    clientSurfaceRevisions.set(clientId, { revision: surfaceRevision, sessionId: context.session.id });
    while (clientSurfaceRevisions.size > MAX_CONTEXT_OWNERS) {
      clientSurfaceRevisions.delete(clientSurfaceRevisions.keys().next().value);
    }
    ...
    scheduleLease();
    if (changed || queueStateChanged) {
      sendTargetContext();
      if (traceId(input.sync_id)) logCommand("context_published", {
        sync_id: traceId(input.sync_id), surface_revision: surfaceRevision,
        context_revision: target.contextRevision,
      });
    }
    ...
    return {
      ...
      accepted_command_sequences: acceptedCommandSequences,
      accepted_result_ids: acceptedResultIds,
      queue: queueEntriesForSession(context.session.id).map(browserQueueEntry),
    };
  }

  function voiceStatus() {
    const connected = socketIsOpen();
    const current = connected ? voiceSnapshot : null;
    const targetActive = Boolean(target && target.expiresAt > now() && target.context.composer.focused);
    const reason = !connected ? "bridge_disconnected"
      : !current ? "state_unavailable"
      : !targetActive ? "no_active_composer"
      : current.target?.session_id !== target.context.session.id
        || current.target.context_revision !== target.contextRevision ? "target_syncing"
      : !current.target.composer.ready ? "composer_unavailable"
      : current.box_connections === 0 ? "box_disconnected" : "ready";
    return { reason, snapshot: current ? structuredClone(current) : null };
  }
