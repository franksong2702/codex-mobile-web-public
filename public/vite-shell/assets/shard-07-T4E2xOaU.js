  ...
  <section class="workspace-stats-section">
	...
	async function loadThreads(options = {}) {
		...
		try {
			...
			const pageThreads = visibleThreads(result.data || []).map((thread) => threadListSummaryFromDetailThread(thread) || thread);
			...
			state.threadListNextCursor = state.selectedCwd && result.mobileWorkspaceHistory === true ? String(result.nextCursor || "") : "";
	...
	function renderThreads(result = null) {
		...
		const moreButton = state.selectedCwd && state.threadListNextCursor ? `<button class="thread-list-load-more" type="button" data-thread-list-load-more>加载更多历史 session</button>` : "";
