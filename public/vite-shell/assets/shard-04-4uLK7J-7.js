...
var STANDALONE_OVERLAY_COMPOSER_STYLE_PROPERTIES = Object.freeze([
	"position",
	"top",
	"bottom",
	"left",
...
function applyStandaloneOverlayComposerLayout(viewport) {
	...
	if (!active) {
		...
		if (previous && typeof previous === "object") for (const property of STANDALONE_OVERLAY_COMPOSER_STYLE_PROPERTIES) composer.style[property] = String(previous[property] || "");
	...
	if (!state.standaloneOverlayComposerInlineStyles) {
		...
		for (const property of STANDALONE_OVERLAY_COMPOSER_STYLE_PROPERTIES) state.standaloneOverlayComposerInlineStyles[property] = composer.style[property] || "";
	}
